...
function Header() {
  ...
  <div className="container relative mx-auto py-2 flex justify-between items-center">
    ...
    <h1 onClick={()=>{navigate(user.isAdmin===true?'/admin':user.isAdmin===false?'/dashboard/student':'/login');setshowDropDown(false)}} className="text-2xl font-bold text-gray-50">ExamMaster</h1>
    <div className=" space-x-10 md:flex hidden">
      ...
      <Link to={'/chatAi'} className="flex items-center space-x-2">
        <span>
          <svg xmlns="http://www.w3.org/2000/svg" className="h-8 w-8 text-yellow-400" fill="none" viewBox="0 0 24 24" stroke="currentColor">
            <path strokeLinecap="round" strokeLinejoin="round" strokeWidth={2} d="M17 20h5v-2a3 3 0 00-5.356-1.857M17 20H7m10 0v-2c0-.656-.126-1.283-.356-1.857M7 20H2v-2a3 3 0 015.356-1.857M7 20v-2c0-.656.126-1.283.356-1.857m0 0a5.002 5.002 0 019.288 0M15 7a3 3 0 11-6 0 3 3 0 016 0zm6 3a2 2 0 11-4 0 2 2 0 014 0zM7 10a2 2 0 11-4 0 2 2 0 014 0z" />
          </svg>
        </span>
        <span className="text-gray-50">Chat wit AI</span>
      </Link>
    </div>
   {showDropDown1 &&ctx.user.login===true &&<div className=" md:hidden bg-white text-black rounded-lg flex flex-col items-left  absolute top-[85%] py-2 w-max space-x-10">
    <ul className="min-w-24">
      <li onClick={()=>setshowDropDown1(false)} className="py-2 px-3 hover:bg-gray-100"><Link className="">Topic</Link></li>
      <li onClick={()=>setshowDropDown1(false)} className="py-2 px-3 hover:bg-gray-100"><Link to={'/notes'} className="">Notes</Link></li>
      <li onClick={()=>setshowDropDown1(false)} className="py-2 px-3 hover:bg-gray-100"><Link to={'/chatAi'} className="">Chat with AI</Link></li>
    </ul>
